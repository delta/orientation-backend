...
func sendAllConnectedUsers(conn *net.Conn, userid int) {
	l := config.Log.WithFields(logrus.Fields{"method": "gobwas/util/sendCastAllConnectedUsers"})
	...
	if err := wsutil.WriteServerMessage(*conn, ws.OpText, responseJson); err != nil {
...
func saveUserNameRedis(userId int, userName string) error {
	l := config.Log.WithFields(logrus.Fields{"method": "gobwas/util/saveUserNameRedis"})
...
func broadcastUserConnectionStatus(userId int, status bool) {
	l := config.Log.WithFields(logrus.Fields{"method": "gobwas/broadcastUserConnectionStatus"})
