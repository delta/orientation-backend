...
func getLeaderBoard(c echo.Context) error {
	...
	query := "SELECT u.userName AS name, L.score AS score, u.department AS department, u.id FROM LeaderBoard AS L LEFT JOIN User AS u ON L.userId = u.id WHERE L.miniGameId = ? ORDER BY L.score DESC;"
